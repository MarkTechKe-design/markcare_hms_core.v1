...
| `AUTH_COOKIE_SAME_SITE`   | Set to `none` when the frontend and API use different hosted sites, such as Vercel and Render.    |
| `AUTH_COOKIE_SECURE`      | Set to `true` in production; required when `AUTH_COOKIE_SAME_SITE=none`.                          |
| `AUTH_COOKIE_DOMAIN`      | Leave empty when frontend and API use unrelated hosted domains.                                   |
...
9. If the frontend and API are on different hosted sites, set backend
   `AUTH_COOKIE_SAME_SITE=none`, `AUTH_COOKIE_SECURE=true`, and leave
   `AUTH_COOKIE_DOMAIN` empty. Redeploy the backend after changing these
   values.
10. Review database storage size with `npm run db:storage:audit` and cleanup
    candidates with `npm run db:cleanup:dry-run`.
11. Validate login, patient search, clinical flows, invoices, PDF downloads,
    M-Pesa sandbox or production callbacks, and admin reports.
12. Move DNS or production traffic only after smoke testing passes.
